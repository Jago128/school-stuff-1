			...
			if (count > n.length - 1) {
				...
				if (searchName(name, n, count) == -1) {
					...
					do {
						...
						if (necessaryCap > cap) {
							System.out.println(
									"No se puede tener una capacidad necesaria mas alta que la capacidad normal.");
						}
				...
				if (next.equalsIgnoreCase("SI")) {
					do {
						...
						if (newCap < n[i].getNecessaryCap()) {
							System.out.println(
									"ERROR. La capacidad no puede ser menor que la capacidad total. Introduzcalo de nuevo.");
						}
